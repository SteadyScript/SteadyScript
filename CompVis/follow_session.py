...
FOLLOW_CONFIG = {
    ...
    "jitter_ref": 3.0,  # Reference jitter threshold (pixels)
    "jerk_ref": 50.0,  # Reference jerk threshold (pixels/s³)
    "wobble_ref": 0.3,  # Reference wobble ratio threshold
    "smoothing_window": 10,  # Frames for moving average
    "rolling_window_fps": 30,  # Frames per second for rolling window
...
class FollowSessionManager:
    ...
    def __init__(self, duration: float = None):
        ...
        Args:
            duration: Session duration in seconds (default from config)
        """
    ...
    def update(self, marker_pos: Optional[Tuple[int, int]], 
        ...
        Args:
            ...
            target_pos: Current target position (x, y) for error calculation (optional)
            dt: Time delta since last frame (default 1/30s)
        """
    ...
    def _compute_final_metrics(self):
        ...
        if len(self.positions) >= 2 and self.total_path_length > 0:
            # Compute smooth path length (from smoothed positions)
    ...
    def _compute_movement_quality_score(self):
        ...
        # Normalize metrics relative to reference thresholds
        ...
        # Combine with weights: 45% jitter, 45% jerk, 10% wobble
        badness = 0.45 * jitter_norm + 0.45 * jerk_norm + 0.10 * wobble_norm
        
        # Convert to score: 100 * (1 - badness/2), clamped to [0, 100]
